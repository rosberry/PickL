...
class SelectionTests: XCTestCase {
    ...
    func testComponentItem_didSelectRowHandler() {
        ...
        var isHandlerCalled = false
        componentItem.didSelectRowHandler = { selectedCcomponentItem, selectedRowIndex, selectedRowItem in
            isHandlerCalled = true

            XCTAssertEqual(selectedRowIndex, 1)
            XCTAssertTrue(componentItem === selectedCcomponentItem)
            XCTAssertTrue(rowItem2 === (selectedRowItem as? RowStringItem))
        }
